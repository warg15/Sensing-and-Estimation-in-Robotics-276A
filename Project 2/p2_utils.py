...
# INPUT 
# im              the map                                                             #the map
# x_im,y_im       physical x,y positions of the grid map cells                        #xim, yim
# vp(0:2,:)       occupied x,y positions from range sensor (in physical unit)         #vector of occupied positions
# xs,ys           physical x,y,positions you want to evaluate "correlation"           #vectors of particle coords
#
# OUTPUT 
# c               sum of the cell values in the map of all the positions hit by range sensor
def mapCorrelation(im, x_im, y_im, vp, xs, ys):
  nx = im.shape[0] #map, number of  x cells
  ny = im.shape[1] #map, number of  y cells
  xmin = x_im[0] #min x coords (-30)
  xmax = x_im[-1] #max x coords (30)
  xresolution = (xmax-xmin)/(nx-1) #resolution, meters per cell (meters/number of cells)
  ymin = y_im[0] #min y coords (-30)
  ymax = y_im[-1] #max y coords (30)
  yresolution = (ymax-ymin)/(ny-1) #resolution, meters per cell (meters/number of cells)
  nxs = xs.size #x size of block passed (81 cells for 9x9)
  nys = ys.size #y size of block passed (81 cells for 9x9)
  cpr = np.zeros((nxs, nys)) #create array 81 x 81
  
  for jy in range(0,nys): #for all y's
    y1 = vp[1,:] + ys[jy] # 1 x 1076     #y1 is first lidar scan plus jyth y coordinate?
    iy = np.int16(np.round((y1-ymin)/yresolution))   #iy a vector that has the number of cells each scan passes through at a given position
    ...
    for jx in range(0,nxs):
      ...
      cpr[jx,jy] = np.sum(im[ix[valid],iy[valid]]) #first dim is x, second is y
  return cpr

#take largest value from this matrix, make that new correlation value, then normalize
