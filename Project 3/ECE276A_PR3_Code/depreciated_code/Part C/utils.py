...
def visualize_trajectory_2d(pose,landmarks, timestamp, path_name="Unknown", show_ori=False):
  '''
  ...
  ax.plot(pose[0,3,:],pose[1,3,:],'b-',label=path_name)
  ...
  ax.grid(False)
  ax.legend()
  fig.savefig(path_name + "t" + timestamp, dpi = 300)
  plt.show(block=True)
  return fig, ax
